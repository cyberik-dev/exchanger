class GetRatesService
  ...
  def set_highest_rate
    highest_rate_index = @data[:for_table].each_with_index.max_by{ |k, i| k[:historical_rate] }.last
    @data[:for_table][highest_rate_index][:highest] = true
  end
  
  def set_lowest_rate
    lowest_rate_item, lowest_rate_index = @data[:for_table].each_with_index.min_by{ |k, i| k[:historical_rate] }
